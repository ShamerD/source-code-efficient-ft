...
if [[ ${TASK} == 'multi_task' ]]; then
  FULL_MODEL_TAG=${MODEL_TAG}_${DATA_TAG}_lr${LR}_s${19}_lora${APPLY_LORA}
else
  FULL_MODEL_TAG=${MODEL_TAG}_${DATA_TAG}_lr${LR}_bs${BS}_src${SRC_LEN}_trg${TRG_LEN}_pat${PATIENCE}_e${EPOCH}_lora${APPLY_LORA}
fi
...
if [[ $APPLY_LORA == 1 ]]; then
  LORA_FN='--apply_lora --lora_alpha '${LORA_ALPHA}' --lora_r '${LORA_R}
fi
