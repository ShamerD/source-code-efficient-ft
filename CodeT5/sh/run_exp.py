...
def get_cmd(task, sub_task, model_tag, gpu, data_num, bs, lr, source_length, target_length, patience, epoch, warmup,
            model_dir, summary_dir, res_fn, apply_lora=False, lora_alpha=None, lora_r=None, max_steps=None, save_steps=None, log_steps=None):
    ...
    if apply_lora:
        cmd_str += ' 1 %d %d' % (lora_alpha, lora_r)
    else:
        cmd_str += ' 0 1 1'
    if max_steps is not None:
        cmd_str += ' %d %d %d' % (max_steps, save_steps, log_steps)
    return cmd_str
