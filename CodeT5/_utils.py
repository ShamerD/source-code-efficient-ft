...
def convert_examples_to_features(item):
    ...
    if args.model_type == 'plbart':
        source_ids = tokenizer(source_str, max_length=args.max_source_length,
    ...
    if stage == 'test':
        ...
        if args.model_type == 'plbart':
            with tokenizer.as_target_tokenizer():
                target_ids = tokenizer(target_str, max_length=args.max_target_length, padding='max_length',
...
def convert_clone_examples_to_features(item):
    ...
    if args.model_type == 'plbart':
        code1 = tokenizer(source_str,
                          max_length=args.max_source_length, padding='max_length', truncation=True)['input_ids']
        code2 = tokenizer(target_str,
                          max_length=args.max_source_length, padding='max_length', truncation=True)['input_ids']
    else:
