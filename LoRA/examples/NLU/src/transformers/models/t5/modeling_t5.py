...
class T5DenseReluDense(nn.Module):
    def __init__(self, config):
        ...
        if config.apply_lora_ff:
            self.wi = lora.Linear(config.d_model, config.d_ff, config.lora_ff_r,
                                  lora_alpha=config.lora_ff_alpha, bias=False)
            self.wo = lora.Linear(config.d_ff, config.d_model, config.lora_ff_r,
                                  lora_alpha=config.lora_ff_alpha, bias=False)
        else:
...
class T5DenseGatedGeluDense(nn.Module):
    def __init__(self, config):
        super().__init__()
        if config.apply_lora_ff:
            self.wi_0 = lora.Linear(config.d_model, config.d_ff, config.lora_ff_r,
                                    lora_alpha=config.lora_ff_alpha, bias=False)
            self.wi_1 = lora.Linear(config.d_model, config.d_ff, config.lora_ff_r,
                                    lora_alpha=config.lora_ff_alpha, bias=False)
            self.wo = lora.Linear(config.d_ff, config.d_model, config.lora_ff_r,
                                  lora_alpha=config.lora_ff_alpha, bias=False)
        else:
...
class T5LayerFF(nn.Module):
    def __init__(self, config):
        ...
        if config.apply_adapter:
            self.adapter = Adapter(config.d_model, config.adapter_size, 'swish' if config.adapter_type == 'houlsby' else 'relu')
    ...
    def forward(self, hidden_states):
        ...
        if hasattr(self, 'adapter'):
            forwarded_states = self.adapter(forwarded_states, residual=forwarded_states)
...
class T5LayerSelfAttention(nn.Module):
    ...
    def forward(
        ...
        forwarded_states = self.dropout(attention_output[0])
        if hasattr(self, 'adapter'):
            forwarded_states = self.adapter(forwarded_states, residual=forwarded_states)
        hidden_states = hidden_states + forwarded_states
...
class T5LayerCrossAttention(nn.Module):
    ...
    def forward(
        ...
        if hasattr(self, 'adapter'):
            forwarded_states = self.adapter(forwarded_states, residual=forwarded_states)
        layer_output = hidden_states + forwarded_states
