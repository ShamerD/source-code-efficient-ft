...
class PLBartAttention(nn.Module):
    ...
    def __init__(
        ...
        if apply_lora:
            self.v_proj = lora.Linear(embed_dim, embed_dim, lora_r, lora_alpha=lora_alpha, bias=False)
            self.q_proj = lora.Linear(embed_dim, embed_dim, lora_r, lora_alpha=lora_alpha, bias=False)
        else:
...
class PLBartEncoderLayer(nn.Module):
    def __init__(self, config: PLBartConfig):
        ...
        if config.apply_lora_ff:
            self.fc1 = lora.Linear(self.embed_dim, config.encoder_ffn_dim,
                                   config.lora_ff_r, lora_alpha=config.lora_ff_alpha, bias=False)
            self.fc2 = lora.Linear(config.encoder_ffn_dim, self.embed_dim,
                                   config.lora_ff_r, lora_alpha=config.lora_ff_alpha, bias=False)
        else:
        ...
        if config.apply_adapter:
            self.adapter_ff = Adapter(self.embed_dim, config.adapter_size,
                                      'swish' if config.adapter_type == 'houlsby' else 'relu')
            if config.adapter_type == 'houlsby':
                self.adapter_attn = Adapter(self.embed_dim, config.adapter_size, 'swish')
    ...
    def forward(
        ...
        if hasattr(self, 'adapter_attn'):
            hidden_states = self.adapter_attn(hidden_states, residual=hidden_states)
        ...
        hidden_states = nn.functional.dropout(hidden_states, p=self.dropout, training=self.training)
        if hasattr(self, 'adapter_ff'):
            hidden_states = self.adapter_ff(hidden_states, residual=hidden_states)
        hidden_states = residual + hidden_states
...
class PLBartDecoderLayer(nn.Module):
    def __init__(self, config: PLBartConfig):
        ...
        if config.apply_lora_ff:
            self.fc1 = lora.Linear(self.embed_dim, config.decoder_ffn_dim,
                                   config.lora_ff_r, lora_alpha=config.lora_ff_alpha, bias=False)
            self.fc2 = lora.Linear(config.decoder_ffn_dim, self.embed_dim,
                                   config.lora_ff_r, lora_alpha=config.lora_ff_alpha, bias=False)
        else:
        ...
        if config.apply_adapter:
            self.adapter_ff = Adapter(self.embed_dim, config.adapter_size,
                                      'swish' if config.adapter_type == 'houlsby' else 'relu')
            if config.adapter_type == 'houlsby':
                self.adapter_self_attn = Adapter(self.embed_dim, config.adapter_size, 'swish')
                self.adapter_cross_attn = Adapter(self.embed_dim, config.adapter_size, 'swish')
    ...
    def forward(
        ...
        hidden_states = nn.functional.dropout(hidden_states, p=self.dropout, training=self.training)
        if hasattr(self, 'adapter_self_attn'):
            hidden_states = self.adapter_self_attn(hidden_states, residual=hidden_states)
        hidden_states = residual + hidden_states
        hidden_states = self.self_attn_layer_norm(hidden_states)
        ...
        if encoder_hidden_states is not None:
            ...
            hidden_states = nn.functional.dropout(hidden_states, p=self.dropout, training=self.training)
            if hasattr(self, 'adapter_cross_attn'):
                hidden_states = self.adapter_cross_attn(hidden_states, residual=hidden_states)
            hidden_states = residual + hidden_states
